...
inputFunction <- function(inputterm) {
  ...
  } else ">Result displays here soon<"
}

########## RANDOM LOOP INPUT - MAIN FUNCTION ##########
## uses randomizer()
...
shinyServer(function(input, output) {
    output$singleOP <- renderText({
        input$singleSubmit
        isolate(
            if (input$singleSubmit == 0) {
                "> Result displays here soon <"
            } else if (input$singleIP == "") {
                "Submit an expression."
            } else inputFunction(input$singleIP)
            )
    })
    output$randomOP <- renderGvis({
        input$randomSubmit
        isolate(
            if (input$randomSubmit == 0) {
                gvisHistogram(data.frame(rep(0, 0)))
            }
            else {
                randomRunFunction(input$randomIP)
            }
        )
    })
})